...
classes = ["1_mediumcircle","4_mediumsquare","6_patch","7_crack"] #顺序要跟voc.names顺序一致
...
def convert_annotation(xml_dir, img_dir):
    ...
    for file in filelist:
        ...
        in_file = open(xml_dir + "/" + file, encoding="utf-8")
        out_file = open(txt_dir + "/" + fileInfor[0] + ".txt", 'w')
        #unicode(fileInfor[0], 'utf-8') # fileInfor[0] = fileInfor[0].decode("UTF-8") #
        imgfile = img_dir + "/" + fileInfor[0] + ".jpg"
        imgfile = os.path.join(img_dir, fileInfor[0] + ".jpg")
        if not os.path.exists(imgfile):
            imgfile = os.path.join(img_dir, fileInfor[0] + ".png")
        else:
            imgfile = os.path.join(img_dir, fileInfor[0] + ".JPG")
        if not os.path.exists(imgfile):
            continue
